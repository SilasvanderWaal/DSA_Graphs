...
pnode add_node(pnode G, char nname) {
    ...
    // Adding a new node to the list IF we do not have a node with the same
    // already
    if (get_name(trailer_node) != nname) {
        pnode new_node = create_node(nname);
        if (!node_cons(trailer_node,
                       new_node)) // If the new node is a new root, trailer_node
                                  // will be NULL. We set G to new node.
            G = new_node;
...
pnode rem_node(pnode G, char name) {
    ...
    // If the construction returns zero, the node to be deleted is root, then we
    // have to move the start pointer
...
// Checks if the new weight is less then the orignial one, if so, the weight is
// updated
...
pedge _rem_edge(pedge E, char to) {
    ...
    // If edge_cons returns NULL, the edge is first in the list and E must be
    // changed
...
void list_to_matrix(pnode G, double matrix[MAXNODES][MAXNODES]) {
    ...
    for (pnode node = G; node != NULL; node = get_next(node)) {
        ...
        for (pedge edge = get_edges(node); edge != NULL;
